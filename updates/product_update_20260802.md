...
* **Creation of Logical Relations**: Two new plugins expand automatic *refersTo* relation creation — a **Reference by Name Comparison** plugin that scores field name similarity using the Jaro-Winkler algorithm, and a **Reference by LLM** plugin that analyzes dataset pairs with an LLM to identify possible foreign-key references.
