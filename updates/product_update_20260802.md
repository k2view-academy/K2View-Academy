...
* **Catalog** has the following major enhancements:
  ...
  * **Creation of Logical Relations** is now enhanced by: a **Reference by Name Comparison** plugin new rule that scores field name similarity using the Jaro-Winkler algorithm, and a new **Reference by LLM** plugin that analyzes dataset pairs with an LLM to identify possible FK references. Both rules can create the relations even for datasets with no PK.
