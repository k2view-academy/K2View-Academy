...
* **Vertical execution mode**: Introducing an additional execution mode for TDM tasks, where instead of executing system by system, the vertical execution processes the **complete LU hierarchy for each root entity** before moving on to the next root entity. This mode is useful when running TDM tasks on **a large scale of entities** as it ensures **better cross-systems data consistency** and **data alignment**.
