...
- **User Profile**: Opens a side pane displaying the current customer context. This pane demonstrates how organizations might use a chat tool to support call center representatives, allowing them to view brief customer information generated by an LLM in natural language. This helps them predict why the customer is reaching out.
- **Chat Summary**: Opens a side pane displaying a summary of the chat so far.
...
2. **Goal Refiner** — determines the intent and refines the goal for the sub-agent
3. **[Domain] Agent** — the specialized sub-agent that handles the request (e.g., 'Credit Card Agent')
...
- **Token consumption**, which is displayed as input tokens, cached tokens, and output tokens.

### Expandable Details

Each of the main workflow stages, representing agents, can be expanded to reveal:

- **System (goals, instructions and context)**: The system prompt, role definition, and objectives given to the agent
- **User**: The user's input
- **LLM Response**: Each iteration of the language model's response, including any tool calls made
- **Tool Calls**: Specific tools invoked throughout the workflow according to the LLM’s instructions
- **Tool Execution Results**: The outcome of each tool call, along with its execution duration
...
3. Executing queries to check current credit card limits (queryExecute)
