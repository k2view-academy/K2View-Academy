...
- **User Profile**: Opens a side pane displaying the current customer context. This pane demonstrates how organizations might apply a chat which is provided to support call center representatives, where they can view brief customer information generated by an LLM in natural language. This can help them predict why the customer is reaching out.
...
Each query displays the processing pipeline of the agentic workflow, showing its stages. For example, when a sub-agent path is chosen, the stages typically are:
