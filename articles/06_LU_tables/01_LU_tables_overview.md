...
*	CDC Indexes - holds a list of table's fields to be published to an external system using the CDC mechanism. This is done per CDC topic, for example "Search", which is used for cross-instance search at Elasticsearch. Read [here](/articles/18_fabric_cdc/05_cdc_consumers_implementation.md) to learn how to configure it.
...
-  **_k2_objects_info**, which stores LU instance statistics.
