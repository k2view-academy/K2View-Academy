...
**LU view** is the result set of a stored query on Fabric DB, which can be queried just as any persistent LU table. This pre-established query command is kept in the Fabric MicroDB dictionary and allows easy access to the required data collection.

LU views are part of the Logical Unit definition, however they are not part of the LU Schema. They are populated with data at the end of the sync process, when the population of all LU tables is completed. 

LU view's column data type is the same as the respective LU table's data type. LU views don't have their own properties and can't be indexed. 
