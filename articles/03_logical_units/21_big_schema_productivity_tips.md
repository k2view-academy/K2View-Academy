...
To do so, first tap the Search box located on the right of the schema's top toolbar. Once done, the search box expands into a **search result widget**, displaying all schema tables in an alphabetical order, along with their related elements (columns and populations). The element type appears beside each element name.
