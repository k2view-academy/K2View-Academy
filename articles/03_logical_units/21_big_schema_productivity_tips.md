...
To do so, first tap the Search box located on the right of the schema's top toolbar. The Search box then expands into a **search result widget**, displaying all schema tables in alphabetical order, along with their related elements (columns and populations). The element type appears beside each element name.
