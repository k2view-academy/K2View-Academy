...
1. [**LU Table**](/articles/06_LU_tables/01_LU_tables_overview.md): Holds and organizes a subset of the data of a Digital Entity including its columns, PKs, indexes and triggers.

2. [**LU Table Population (ETL)**](/articles/07_table_population/01_table_population_overview.md): 
    * Imports data to the LU table from a variety of data sources and keep it up to date.
    * Different types of data manipulations like enrichment, cleansing, masking, transforming, etc. can be carried out.  
3. [**LU Schema**](/articles/03_logical_units/03_LU_schema_window.md): This is the heart of an LU. The schema determines the relationship between the LU tables (similar to foreign keys). An LU schema has one LU table defined as its Root Table that holds the Digital Entity’s unique key.
