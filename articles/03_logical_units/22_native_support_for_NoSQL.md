...
* LU tables are created with the following system-generated fields:
  ...
  * ```_value``` is added only to LU tables that represent an array of primitives, to store the element’s value. Note that [Catalog masking](/articles/39_fabric_catalog/catalog_app/11_catalog_masking.md) does not apply to arrays of primitives (arrays containing only primitive values such as numbers or text strings). If these values need to be masked, the implementor must apply the masking logic manually.
