...
8. Edit the **Basic SAML Configuration** section (1) - information should be taken to match the Fabric Configuration, as explained [here](/articles/26_fabric_security_iam/13_user_IAM_configuration.md#saml-configuration).
...
11. Upload the public key certificate used to encrypt the SAML assertion, as exported from Fabric. Read more [here](/articles/26_fabric_security_iam/13_user_IAM_configuration.md#saml-configuration) in SAML Configuration > Preparations > Provide to the IDP. 
...
In addition to the instructions detailed [here](/articles/26_fabric_security_iam/13_user_IAM_configuration.md#saml-configuration), setting up SAML with Azure AD requires adding a configuration parameter to the config.ini file: `SECURITY_WANT_NAMEID_ENCRYPTED=false`
