...
- **Authentication response** - This is a token or assertion (in XML format) that contains authentication information about the user/principal, such as:
  ...
  - **Conditions** - For example: valid time frame (including start date, expiration date, and issuing date).
  - **Attribute statements** - These statements contain a list of attributes in a name-value pair form, for example, the groups or roles associated with the user.
- **Assertion consumer service (ACS)**  - The service at the SP that receives the SAML response. The URL of the ACS is provisioned at the IDP settings stage and is also sent upon authentication request. 
...
* **Encryption** - SAML supports several encryption methods, with SHA-256 being the most commonly used (this is also adopted by Fabric).

* **Trust** - SP Entity ID and IDP Entity IDs are transferred upon request and response for trust purposes. Additionally, any request is sent with an ID, which the SP can then verify upon response. 
