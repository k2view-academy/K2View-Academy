...
Output from the Swagger GUI for grSQLGldPlus.graphit with the Customer Instance ID = 1234:

<img src="/articles/15_web_services_and_graphit/17_Graphit/images/59_graphit_examples.PNG" width="700" height="400"></img>


Output from the Swagger GUI for grSQLBrz.graphit with the Customer Instance ID = 1000:
...
#### grFormat.graphit
In this example the children nodes of the CRM_DB node are defined as a field. The response populates the document with the names and values of each specific field.<br></br>
...
#### grGroup.graphit
The ${x} string has been added to both TRUE and FALSE groups whereas the ${y} value is declared externally. The displayed ${x}   also lists the group of origin.<br></br>
<img src="/articles/15_web_services_and_graphit/17_Graphit/images/14_graphit_examples_tags.PNG" width="700" height="400"></img>

#### grCollect.graphit
This example illustrates how both Subscriber and Billing datasets are collected into one single array.<br></br>
...
#### grOne.graphit
The **one** flag is set to TRUE and has been applied to the Billing_DB2 node. The response only returns the first value for {"BILLING_DB2":{"SUBSCRIBER_ID":2}}, instead of the 10 values expected for this tag had the **one** flag not been activated).<br></br>
...
#### grShowEmpty.graphit
The **showEmpty** flag has been set to False and been applied on the CRM_DB node. Empty nodes are not shown in the response.<br></br>
<img src="/articles/15_web_services_and_graphit/17_Graphit/images/24_graphit_examples_tags.PNG" width="700" height="400"></img>random number

#### grShowNull.graphit
The **showNull** flag has been set to False and applied to the CRM_DB node. Null values will therefore be ignored and not shown in the part of the response referring to the CRM_DB. Since the flag has not been applied to the Billing_DB node, null values are displayed.<br></br>
...
#### grCSV.graphit
The csvRow has been set to the SUBSCRIBER_ID node and the csvHeader has been set to False in the Subscriber_info node. The header has been removed from the CSV output and a new line has been created for each new subscriber_id entry.<br></br>
