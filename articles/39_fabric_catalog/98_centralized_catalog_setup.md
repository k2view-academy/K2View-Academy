...
- The Fabric Discovery and Catalog solution requires **Neo4j Enterprise edition**. For the exact certified Neo4j version, refer to the latest Fabric Release Notes.
- All Fabric instances linked to a single Neo4j GraphDB must have **the same Fabric version**. Pointing different Fabric versions to the same Neo4j instance is not supported.
