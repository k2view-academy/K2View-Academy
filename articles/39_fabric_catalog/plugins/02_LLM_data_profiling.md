...
- ```"userPrompt"``` is an LLM prompt definition. It is a dynamic string, comprised of several parts that are combined at run time. Some of them are taken from the framework and some are taken from the plugin's definition, as follows:
