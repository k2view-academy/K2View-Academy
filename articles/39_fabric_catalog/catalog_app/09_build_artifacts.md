...
Starting from V8.3, the artifacts of the relations can be created, when needed. This is only available by the ```/api/catalog/{version}/build-catalog-artifacts``` API, by setting ```refersTo=true``` in the API's input, as described [here](/articles/39_fabric_catalog/20_catalog_APIs.md#build-catalog-artifacts). 
