...
The Discovery job has two main parts: Crawler and Plugin Framework. The execution is performed based on the product configuration **plugins.discovery** file which defines the list of active plugins, their execution order, threshold and other input parameters: 
...
Starting V8.2, the Discovery execution is based on the rules. The system includes a **BaselineRule** which defines a baseline product configuration. In addition, the user can create one or more rules, per each relevant data platform. Each rule can define:
...
All rules are saved on the project level under the ```Implementation/SharedObjects/Interfaces/Discovery/``` folder. 
