...
The master key (AES-256) generated by Fabric is broken into bytes. Each byte is stored in a separate record in a dedicated Cassandra table. Then, using Cassandra distribution data logic, each node stores only part of the master key. In addition, a master key rotation scheme, allows users to regularly change the master key. New LUIs are encrypted with the new key, while previous ones are re-encrypted with the new key during the next synchronization.
