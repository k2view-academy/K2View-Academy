...
Fabric Sync is a mechanism that synchronizes data between data sources and the Fabric database by the extraction and transformation processes executed on an [LU Instance (LUI)](/articles/01_fabric_overview/02_fabric_glossary.md#lui). Fabric sync can be performed in two modes:
