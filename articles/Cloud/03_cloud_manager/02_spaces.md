...
> The Space creation process may vary between project and profiles. For example, for non-Studio Spaces, the Space creation includes the step of build and deploy execution as well, which may take time. along with deploying, the deployment process also entails the execution of some further automatic processes, defined either in [deploy.flow](/articles/19_Broadway/09a_automatic_flows_execution_upon_deploy.md) or as user jobs, which may take slightly longer. 
