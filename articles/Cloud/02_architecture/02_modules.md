...
## K2agent and K2deployer

K2cloud contains 2 modules, reside inside each site, and aimed to be the connection and the interaction points with the Orchestrator. Having that K2cloud platform has no access to the Fabric spaces and the data managed by them, they are located in a separated name-space inside the K8s cluster. 

* **K2agent** is the element inside each K8s cluster / site which interacts with the K2cloud Orchestrator, for creating, deleting, pausing, deploying project and get status of the spaces that held inside this site. It functions as simple "pipe" that pulling messages from K2cloud platform via REST call from the site's dedicated "mailbox". and move the messages to the K8s control plain. 
