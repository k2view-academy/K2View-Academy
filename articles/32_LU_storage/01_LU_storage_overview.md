...
Optimistic locking is supported for all storage types (except GCS). If needed, it can be switched ON by setting ```VALIDATE_REMOTE_VERSION=true```.

Note that Fabric uses the Cassandra DB as a03 default system management database. Therefore, changing the LU Storage from Cassandra DB to one of the above storage types doesn't replace the Fabric need for other persistent storage. 
