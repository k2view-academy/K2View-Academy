...
These transactions can be populated in the iidFinder delta table, where the delta table is created under the [System DB **k2staging** schema](/articles/02_fabric_architecture/06_cassandra_keyspaces_for_fabric.md). The Sync process then triggers the iidFinder to get the transactions from the delta table and updates the LUI.
