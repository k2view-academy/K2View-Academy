...
By default, Fabric uses its built-in master-key generator which uses strong Java methods aimed for AES key generation algorithms. 
Mater key can be regenerated via the [Master Key Rotation](#master-key-rotation) mechanism.

An alternative for using its built-in master key generator, Fabric enables integration with [KMS](#kms) (Key Management Service) to generate and encrypt its master key.   
...
For decrypting a data, the application shall take the encrypted data key from its storage, call to KMS to decrypt it into its clear/plain form, and use it for decrypting the data.

#### Fabric Working-flow with KMS 

When a KMS integration is used, Fabric treats KMS's data-key as its master key: 

When integrated with AWS, instead of generating master key it calls to AWS KMS to get a data key within its 2 forms. When integrated with GCP, it generates a data key and calls to GCP KMS to encrypt it. Then, Fabric treats the *encrypted* data key as its master key.  
...
> Note: in any point these commands can be executed with these parameters or the defaults - self generated and self stored master key - and since then encryptions will be done with the newest, where Fabric will be able to decrypt data which was encrypted with previous keys, of any type. For example, if Fabric already works with a self generated master key and in some point it is required to adopt KMS, then configuration shall be set at the config.in and then relevant above ``activatekey`` command (AWS, GCP) shall be executed. 

#### Key Rotation

When working with KMS there are two levels or key rotation: The data key rotation and the KMS master key rotation. 

* Data key rotation, which is used as Fabric master key, is done at Fabric using the ``activatekey `` command and is described [here](#master-key-rotation).
* KMS's master key rotation is done at KMS, and is done according to the master key settings - automatically or manually. This rotation is transparent to Fabric, so that it decrypts also data keys, sent to by Fabric, which were encrypted with older master keys.
