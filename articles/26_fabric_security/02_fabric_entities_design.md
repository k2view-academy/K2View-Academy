...
When integrated with AWS, instead of generating ש master keyת it calls to AWS KMS to get a data key within its 2 forms. When integrated with GCP, it generates a data key and calls to GCP KMS to encrypt it. Then, Fabric treats the *encrypted* data key as its master key.  
