...
* **Get from the IDP** - to be copied and downloaded from the IDP admin user interfaces 
  ...
  * Certificate file - the public key which is generated by the IDP. To import it into Fabric truststore run the following:
    ...
    The path to the trust-store file and related parameters can be found in the jvm.options file: At the *TLS/SSL SETTINGS* section look for "javax.net.ssl.trustStore" . All those values are set there for a hardened environment. An example to such path is: "$K2_HOME/.cassandra_ssl/cassandra.truststore"
