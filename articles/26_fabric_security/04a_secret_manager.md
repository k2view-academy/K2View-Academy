...
The following are the required config.ini attributes for each Secrets Management provider:
...
Authentication and authorization can be done by the service account that the server is associated with. This is an alternative to using an Access ID and an Access Key.
