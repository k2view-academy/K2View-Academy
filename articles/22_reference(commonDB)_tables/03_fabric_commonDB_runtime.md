...
A reference backup is stored in the same storage as defined in the DEFAULT_GLOBAL_STORAGE_TYPE parameter in the config.ini it should be shared storaged to be available for all nodes in the cluster, unless the COMMONS_BACKUP_DEFAULT_STORAGE parameter is set up in the config.ini.
