...
The transaction is sent to Kafka and is saved into the Kafka or Cassandra, depending on its size
...
## Sync Process
If the truncate option is set:
- Start snapshot
- Add all rows to the snapshot
- If any error or exception rollback 
- Else commit – in a single transaction:
  - Create indices on the temporary table.
  - Drop an old reference table.
  - Rename the temporary table.

If the truncate option is not set: 
- Run updates for table 
