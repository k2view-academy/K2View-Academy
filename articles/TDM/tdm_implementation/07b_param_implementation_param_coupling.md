...
- Verify that the linked fields are defined as a PK or a unique index in the parent LU table in order to support the MDB export of these tables. All
 the parent LU table's PK/unique index fields must be linked to the child LU table. This is required to create the FK relation in the PG DB for the exported LU tables. 
