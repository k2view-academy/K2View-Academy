...
-  The data generation flow returns multiple results that serve as the row columns. This means that the [rowsGenerator Actor](/articles/19_Broadway/actors/07a_data_generators_actors.md#rowsgenerator) handles the loop on parent rows and number of table's records that each parent will get.
