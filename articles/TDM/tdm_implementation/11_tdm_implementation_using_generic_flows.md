...
Set the **TDM_SEQ_REPORT** Global to **false** to prevent the population of TDM_SEQ_MAPPING and the generation of the **Replace Sequence Summary Report** for a better performance. Note that the replace sequence report will not be available for a task that is executed with the TDM_SEQ_REPORT as **false**.
