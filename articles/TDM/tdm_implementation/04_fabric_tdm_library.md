...
Import and deploy all TDM Web Services (APIs) to the Fabric project. These Web Services are invoked by the TDM Portal application and comprise the back-end layer of the TDM Portal application.
...
- **TDM 7.6 includes the TDM Portal code in the TDM LU**: the TDM Portal code is kept under the web sub folder in the TDM LU. Note that the web directory can only be viewed using the Fabric web studio. If you use the desktop, you can right-click the TDM LU > Open Folder and view the web folder in the windows File Explorer.
