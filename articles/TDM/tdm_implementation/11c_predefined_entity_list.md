...
This article guides how to implement Custom Logic flow in order to enable an entity subset based on the [Predefined Entity List](/articles/TDM/tdm_gui/15a_entity_subset.md#predefined-entity-list) in a task. The entity list of the full entity subset can be generated by either using an SQL query on the source DB or running a Broadway flow. A Broadway flow is needed when running an extract task on a non-JDBC data source.  
