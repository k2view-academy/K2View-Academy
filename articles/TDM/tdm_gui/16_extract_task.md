...
- The number of processed entities will not exceed the number of entities returned by the custom logic flow. For example: if the maximum number of entities in the task is 50, but the custom logic only returns 30 entities, the task will process 30 entities.
