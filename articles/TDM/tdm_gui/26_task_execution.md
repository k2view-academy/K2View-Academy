...
A task can be executed multiple times. The [Task Execution process](/articles/TDM/tdm_architecture/03_task_execution_processes.md) can be initiated by either clicking the play icon ![task execution icon](images/execute_task_icon.png) in the portal, calling the [start task execution API](/articles/TDM/tdm_gui/TDM_Task_Execution_Flows_APIs/04_execute_task_API.md), or by the TDM scheduling process if the task's [scheduler](22_task_execution_timing_tab.md) is defined.
...
- Execute and extract the task with the following LUs:
  ...
  - Billing - this is the child LU of the Customer LU.
...
- The Billing LU is executed after the execution of the Customer LU has ended:
  ...
  **Example 2 - Vertical Execution:**
  ...
  - The Customer LU is displayed as it is the root LU. The related Billing entities are executed behind the scenes for each executed customer before moving to the next customer ID. Both LUs are marked as running:
...
- Click ![resume](images/resume_execution_icon.png) next to a record with a **stopped** execution status in the **Logical Unit Summary** to resume the execution of **all** stopped task's LUs and post-execution processes.
