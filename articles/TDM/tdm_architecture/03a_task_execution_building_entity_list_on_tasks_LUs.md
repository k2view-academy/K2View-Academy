...
- Generating a list of **dummy entities** for a **synthetic data generation** task. The **Synthetic** is concatenated to each generated ID to create a dummy LUI. For example, Synthetic_84.

  Click [here] for more information about synthetic data generation tasks.

The Broadway flows get the entities for the task execution and inserts them into a dedicated Cassandra table in the **k2_tdm** keyspace. A separate Cassandra entity table is created on each LU and has the following naming convention: [LU_NAME]_entity_list. The task execution process uses this table to select the entity list for the task execution.
