...
It is highly recommended to avoid duplicating names of Shared Objects at lower levels of a project, for example, within a Logical Unit component. However, if there are duplicate object file names, the file residing under the specific lower-level component takes priority upon execution.
...
* If a new category name is entered into the Category setting, a new **category** will be created, and the function will be associated with the new Logic.java file.

Each category creates a separate Logic.java file, which has a specific path, thereby differentiating it from other files in other categories.
