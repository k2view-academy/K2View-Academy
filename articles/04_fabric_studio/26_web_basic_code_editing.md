...
Web Studio is a full, professional and comprehensive IDE, Visual Studio Code like editor. If you are not familiar with source code editors you can watch and learn from VS Code's user guides, but keep in mind that Web studio is not fully the the same (as it is based on Code OSS, the open source project behind VS Code).
