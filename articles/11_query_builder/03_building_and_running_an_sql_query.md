...
* Select the **DB objects** and click on **Add Selected** (at the bottom of the DB Tree pane).
  The selected DB objects are then added onto the Main window, and the related SQL statement displays in the [Query window](/articles/11_query_builder/02_query_builder_window.md#query-tab).
...
Note: Such removal automatically updates the SQL statement in the Query window.
...
5. The query will then be generated and will appear in the SQL Editor.
   ...
   You can see the AI Assistant's explanations, on how SQL command was created, by clicking on the question mark icon that appears above the SQL Editor, once the Text to SQL generation process has ended.
