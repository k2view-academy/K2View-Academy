...
* **interface** - the interface to be used to cache the masked values. This interface may be either any SQL DB interface defined in Fabric, Fabric server memory, or **SEED**. When the Interface parameter is populated with **SEED**, the Masking Actor uses the [Data Consistency Using Seed](/articles/26_fabric_security/06_data_masking.md#data-consistency-using-seed) method for data consistency: it populates the caching key into the seed parameter and sends it to the data generation Actor. 
