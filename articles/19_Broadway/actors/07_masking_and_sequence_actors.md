...
* **useInstanceId** - indicates whether to use the Instance ID as part of the masking cache. Note that from Fabric 7.1 onwards gets the instance id value from the root_iid key if it is set. If the root_iid key is not set, it gets the current LUI instance. The root_iid key enables to maintain the referential integrity on PII fields across different LUs that logically belong to each other. For example, CRM and Billing LUs keep the Customer's data. The customer name needs to be idendical in both LUs for  a given customer. Setting the root_iid with the customer ID enables keeping the referential integrity betweegn the CRM and Billing LUs.
