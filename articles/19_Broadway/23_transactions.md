...
The use case for using this Actor is [TDM Clone Entity](/articles/TDM/tdm_gui/17_load_task_regular_mode.md#entity-clone-synthetic). The Entity Clone task gets one LU instance of the cloned entity and each Clone reads the data from a single LUI instance. We need to exclude the select of the LUI from the transaction to enable running several Clone processes in parallel and avoid locking the LUI. It requires Fabric to be marked as non-transactional interface in order to prevent locking of the Instance SQLite file while the transaction is open on target DB.
