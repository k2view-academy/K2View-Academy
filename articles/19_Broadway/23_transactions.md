...
Starting from Fabric V6.5.8, an interface can be marked as non-transactional in the context of an active transaction using the **NoTx** Actor. The purpose of the **NoTx** Actor is to exclude an interface from a transaction in order not to execute the **begin**, **commit** and **rollback** commands.
