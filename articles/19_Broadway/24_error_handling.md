...
Advanced error handling features can be achieved using an **ErrorHandler** and **ErrorFields** Actors that can allow to configure different kinds of behavior depending on the exception type. To learn about the advanced error handling options, refer to the [Error Handling Actors](actors/06_error_handling_actors.md) article.
